...
<template>
  <div class="flex flex-row justify-center space-x-10 bg-slate-500 h-screen pt-2 pb-2">
      <div class="container bg-indigo-500 h-full w-1/4 rounded-lg">
          ...
          <div class="flex flex-col pl-10 pr-10 rounded-lg mt-6 relative bg-slate-100" style="height: 750px; overflow-y: scroll;">
...
export default {
  ...
  mounted() {
    this.getBudgets();
    console.log("this.budgetsArr: ", this.budgetsArr);
    // this.details = [];
    this.inc = this.budgetsArr.income;
    this.exp = this.budgetsArr.expense;
    this.total = this.budgetsArr.total;
    console.log("this.budgetsArr.transaction: ", this.budgetsArr.transaction);
    // if (this.budgetsArr.transaction.length > 0) {
    //     this.budgetsArr.transaction.forEach(trans => {
    //         this.details.unshift(`
    //           <div class="flex items-center">
    //               <span class="material-icons ${trans.class}">${trans.icon}</span>
    //               <p>${trans.text}</p>
    //           </div>
    //           <div class="flex items-center">
    //               <p style="color: ${trans.color}">${trans.type} ${trans.current}</p>
    //               <button id=${trans.id} class="transaction__bin">
    //                 <span class="material-icons" onclick="helo">delete</span>
    //               </button>
    //           </div>`);
    //     })
    // }
    // const items = this.$refs.transaction_list.children;
    let items = document.querySelectorAll(".transaction__bin")
  ...
  methods: {
    helo() {
      ...
      selectedItem(category, e) {
        ...
        //   material-icons p-2 bg-slate-200 rounded-3xl
      },
      getColor(item) {
